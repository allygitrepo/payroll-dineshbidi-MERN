...
export const updateSaasClient = async (id, clientData) => {
  try {
    const response = await apiClient.put(`/saas/clients/${id}`, clientData);
    return response.data.data;
  } catch (error) {
    throw error;
  }
...
export const getEmployeeReports = async () => {
  try {
    const response = await apiClient.get('/saas/employee-reports');
    return response.data.data;
  } catch (error) {
    throw error;
  }
